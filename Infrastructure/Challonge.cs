...
//            Username = WebConfig.GetAppSetting("challongeUserName");
  //          Key = WebConfig.GetAppSetting("challongeKey");
         ...
         * 
         * */
        private string Load(string apiMethod = "", string method = "GET", string filePath = "")
        {
            try
            {
                using (var webClient = new WebClient())
                {
                    string URL = Url + apiMethod + ".json";
                    webClient.Credentials = new NetworkCredential(Username, Key);
                    ...
                    string toReturn = webClient.DownloadString(URL);
                    SetResultOk();
                    return toReturn;
                }
            }
        ...
        * 
        * Parameters:
        * <param name="TournamentCreation">
        *   The TournamentCreation object to be passed to the Challonge's Website
        * </param>
        * 
        * Return Value:
        * <returns>
        ...
        **/
        ...
        public ChallongeTournament FinalizeTournament(string tournamentUrl)
        {
            return ProcessJson<ChallongeTournament, JsonChallongeTournament>(
                   JsonConvert.DeserializeObject<JsonChallongeTournament>(
                       Send("", "POST", "/" + tournamentUrl + "/finalize", false)
                   )
                 );
        }
        ...
        public IEnumerable<ChallongeParticipant> AllParticipants(string tournamentUrl)
        {
            ...
            IList<ChallongeParticipant> participantResults = ProcessResults<ChallongeParticipant, JsonChallongeParticipant, JArray>(participants);
      ...
      /* NEEDS WORK - BulkCreateParticipants
        ...
        */
        ...
        public IEnumerable<MatchChallonge> AllMatches(string tournamentUrl)
        {
            IEnumerable<JsonMatchChallonge> jsonList = JsonConvert.DeserializeObject<IEnumerable<JsonMatchChallonge>>(
                                                               Load("tournaments/" + tournamentUrl + "/matches"));
